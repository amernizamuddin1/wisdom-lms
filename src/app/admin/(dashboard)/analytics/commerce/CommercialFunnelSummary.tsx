...
export default function CommercialFunnelSummary({ stages }: { stages: CommercialFunnelStageRow[] }) {
      ...
      <CardContent>
        {allZero ? (
          <p className="py-10 text-center text-sm text-muted-foreground">
            No commercial funnel activity matches the selected filters yet.
          </p>
        ) : (
        ...
        <p className="mt-4 text-xs text-muted-foreground">
          Product Viewed, Added to Cart, Checkout Started, and Payment Initiated only count authenticated visitors and
          only have data from when this feature launched — Payment Completed has full history.
        </p>
      </CardContent>
    </Card>
  );
