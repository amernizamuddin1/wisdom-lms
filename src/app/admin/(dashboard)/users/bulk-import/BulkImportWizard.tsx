...
const parseInitial: ParseUserCsvState = {};
const confirmInitial: ConfirmImportState = {};

export default function BulkImportWizard() {
  const [parseState, parseAction, parsePending] = useActionState(parseUserCsv, parseInitial);
      ...
      <Card>
        <CardContent className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
            ...
            <Stat label="Created / Added" value={confirmState.successCount ?? 0} variant="success" />
            <Stat label="Skipped (already a member)" value={confirmState.skippedCount ?? 0} />
        ...
        <CardContent className="space-y-4">
          ...
          <div className="flex gap-4 text-sm">
            <span className="text-success">
              {parseState.validCount} valid
            </span>
            <span className="text-muted-foreground">{parseState.existingCount} existing account (will join tenant)</span>
            <span className="text-muted-foreground">{parseState.alreadyMemberCount} already a member (skip)</span>
                  ...
                  <tr key={row.rowNumber}>
                    ...
                    <td className="px-3 py-2">
                      {row.errors.length > 0 ? (
                        ...
                        <Badge variant="outline">Skip (already a member)</Badge>
                      ) : row.existsAlready ? (
                        <Badge variant="success">Add existing account to tenant</Badge>
                      ) : (
      ...
      <CardContent className="space-y-4">
        <div className="flex items-center justify-between">
          ...
          <Button
            ...
            size="sm"
            onClick={async () => {
              const csv = await downloadUserCsvTemplate();
              triggerCsvDownload(csv, "bulk-user-import-template.csv");
            }}
          >
            Download Template
          </Button>
