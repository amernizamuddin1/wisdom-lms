...
import { uploadCommunicationImage, type UploadImageState } from "./emails/actions";
import { ctaButtonHtml } from "@/lib/email-templates/layout";
...
export default function EmailRichEditor({
  ...
  const [ctaPromptOpen, setCtaPromptOpen] = useState(false);
  ...
  function insertCta() {
    if (!ctaUrl.trim() || !ctaLabel.trim()) return;
    editor!.chain().focus().insertContent(ctaButtonHtml(ctaLabel.trim(), ctaUrl.trim(), primaryColor)).run();
    setCtaPromptOpen(false);
  ...
  async function handleImageUpload(file: File) {
    ...
    try {
      ...
      const result: UploadImageState = await uploadCommunicationImage({}, formData);
      if (result.url) {
  ...
  return (
    <div className="space-y-2">
      <div className="flex flex-wrap items-center gap-1 rounded-md border bg-muted/40 p-1.5">
        ...
        <ToolbarButton label="Divider" active={false} onClick={() => editor.chain().focus().setHorizontalRule().run()}>—</ToolbarButton>
        ...
        <ToolbarButton label="CTA Button" active={false} onClick={() => setCtaPromptOpen(true)}>Button</ToolbarButton>
        ...
        {ctaPromptOpen && (
          <span className="flex flex-wrap items-center gap-1">
            <input
              ...
              className="w-28 rounded border border-input bg-transparent px-2 py-1 text-xs text-foreground outline-none focus-visible:border-ring focus-visible:ring-[3px] focus-visible:ring-ring/50"
            />
            <input
              type="text"
              value={ctaUrl}
              ...
              placeholder="https://..."
              className="w-40 rounded border border-input bg-transparent px-2 py-1 text-xs text-foreground outline-none focus-visible:border-ring focus-visible:ring-[3px] focus-visible:ring-ring/50"
            />
            <button type="button" onClick={insertCta} className="rounded bg-primary px-2 py-1 text-xs font-medium text-primary-foreground hover:bg-primary/90">Insert</button>
            <button type="button" onClick={() => setCtaPromptOpen(false)} className="rounded px-2 py-1 text-xs text-muted-foreground hover:bg-muted">Cancel</button>
          </span>
        )}
      </div>
      ...
      <EditorContent
        editor={editor}
        className="prose prose-sm min-h-[280px] max-w-none rounded-md border border-input px-3 py-2 text-sm text-foreground [&_.ProseMirror]:min-h-[260px] [&_.ProseMirror]:outline-none [&_img]:max-w-full"
      />
